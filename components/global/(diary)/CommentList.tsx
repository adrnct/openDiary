...
const CommentList = async ({ diary_id, user }: ParamsProps) => {
    ...
    const { data, error } = await supabase.from('diary').select('comments').eq('id', diary_id).single()
    ...
    return (
        <div className='flex flex-col gap-4'>
            ...
            {data.comments.map((comment: IComments, index: number) => {
                return (
                    <div key={comment.comment_id} className='mx-4 card card-body card-bordered p-4 bg-base-200'>
                        <div className='md:flex justify-between gap-4 items-center'>
                            <div className='flex gap-4'>
                                ...
                                <div>
                                    <div className='md:flex gap-1 items-center mb-2'>
                                        <p className='font-bold md:text-lg'>{comment.username || comment.email}</p>
                            ...
                            <div className='flex justify-end'>
                                {user == email || user == username || comment.email == email ? <DeleteCommentButton diary_id={diary_id} comment_id={index} /> : null}
                            </div>
                        </div>
                    </div>
                )
