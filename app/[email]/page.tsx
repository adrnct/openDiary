...
const page = async ({ params }: ParamsProps) => {
    ...
    const { data, error } = await supabase.from('user').select().order('created_at', { ascending: false }).eq('email', decodedEmail).single()
                        ...
                        <div className='flex'>
                            <div className='flex-1'>
                                <div className='flex justify-center items-center h-44 bg'>
                                    <div className='gap-2'>
                                        <p className='text-2xl text-center'>{data.content.length}</p>
